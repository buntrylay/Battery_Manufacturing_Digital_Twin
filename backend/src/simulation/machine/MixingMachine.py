...
class MixingMachine(BaseMachine):
    ...
    def _format_result(self, is_final=False):
        ...
        with self.lock:
            ...
            composition = {
                ...
                f"{self.slurry.solvent}": round(
                    getattr(self.slurry, self.slurry.solvent), 3
                ),
            }
            properties = {
                ...
                "YieldStress": round(
                    self.calculator.calculate_yield_stress(self.slurry), 2
                ),
            }
            if is_final:
    ...
    def __mix_component(self, component, step_percent, pause_sec):
        """
    ...
    def run(self, step_percent=0.02, pause_sec=0.1):
        ...
        if self.is_on:
            for comp in ["PVDF", "CA", "AM"]:
                self.__mix_component(comp, step_percent, pause_sec)
