...
class CoatingMachine(BaseMachine):
    ...
    def __init__(self, id, collector_foil: CollectorFoil, machine_parameters: dict):
        """
        Initialize the coating machine.
        
        Args:
            id (str): Unique identifier for the machine
            collector_foil (CollectorFoil): The collector foil object
            machine_parameters (dict): Dictionary containing machine parameters
        """
        super().__init__(id)
        self.start_datetime = datetime.now()
        self.total_time = 0
        self.lock = threading.Lock()
        self.collector_foil = collector_foil
        ...
        self.viscosity_pa = collector_foil.viscosity
    ...
    def _format_result(self, step=None, is_final=False):
        ...
        with self.lock:
            ...
            properties = {
                ...
                "wet_thickness_m": round(self.collector_foil.wet_thickness, 5),
                "dry_thickness_m": round(self.collector_foil.dry_thickness, 5),
                "defect_risk": self.defect_risk,
                "uniformity_std": round(self.collector_foil.uniformity, 4)
            }
    ...
    def _simulate(self, end_time=100):
        ...
        for t in range(0, end_time+1, 5):
            ...
            # Update collector foil properties
            self.collector_foil.wet_thickness = self.calculator.calculate_wet_thickness(self.flow_rate, self.coating_speed, self.coating_width)
            self.collector_foil.dry_thickness = self.calculator.calculate_dry_thickness(self.collector_foil.wet_thickness, self.solid_content)
            self.collector_foil.uniformity = self.calculator.calculate_uniformity(self.shear_rate)
