...
class CoatingModel(BaseModel):
    def __init__(self, mixing_model: MixingModel):
        ...
        total_solids = mixing_model.AM + mixing_model.CA + mixing_model.PVDF # total solids volume, taken from mixing model's AM, CA, PVDF
        total_volume = total_solids + mixing_model.solvent # total volume, taken from mixing model's AM, CA, PVDF, solvent
    ...
    def update_properties(self, flow_rate, coating_speed, coating_width, gap_height):
        ...
        self.defect_risk = self.calculate_defect_risk(coating_speed, gap_height, self.viscosity)
