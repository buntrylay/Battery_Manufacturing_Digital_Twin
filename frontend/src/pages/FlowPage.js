...
function FlowPage() {
  ...
  const handleStartFullSimulation = async () => {
    ...
    try {
      // Use the new continuous batch simulation API
  ...
  const handleResetPlant = async () => {
    // Using window.confirm is okay for simple confirmations, but for a better UX,
    // you might consider a custom modal component in the future.
    if (
      !window.confirm(
...
// Correct: Export the component function itself, don't call it.
