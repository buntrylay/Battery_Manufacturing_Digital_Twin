...
const SidePanel = ({ selectedStage, onClose }) => {
  ...
  const isMixingStage = selectedStage ? (selectedStage.id === "Anode Mixing" || selectedStage.id === "Cathode Mixing") : false;
  ...
  const handleSaveInputs = () => {
    ...
    try {
      ...
      localStorage.setItem(`mixingInputs_${selectedStage.id}`, JSON.stringify(savedData));
  ...
  const currentSum = isMixingStage ? 
    Object.values(inputValues).reduce((acc, val) => acc + parseFloat(val || 0), 0) : 0;
